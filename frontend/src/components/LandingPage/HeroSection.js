...
const HeroSection = ({ onSignupClick, isDarkMode }) => (
  <section className={`min-h-screen w-full flex items-center ${isDarkMode ? 'bg-gray-900' : 'bg-white'} transition-colors duration-300`}>
    <div className="container mx-auto px-4 lg:px-8 flex flex-col-reverse lg:flex-row items-center justify-between gap-8 py-8 lg:py-0">
      {/* Left Content */}
      <div className="lg:w-1/2 w-full text-center lg:text-left">
        <h1 className={`text-4xl lg:text-5xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'} mb-6 transition-colors duration-300`}>
          Order Your Favourite<br />Food Easily
        </h1>
        <p className={`text-lg mb-8 max-w-xl mx-auto lg:mx-0 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'} transition-colors duration-300`}>
