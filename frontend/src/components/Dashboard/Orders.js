      ...
      {isLargeView ? (
          ...
          {!Array.isArray(orders) ? (
            ...
            <motion.div
              ...
              className={`text-center py-8 rounded-lg border ${
                isDarkMode ? 'bg-gray-800 text-gray-400 border-gray-700' : 'bg-gray-50 text-gray-500 border-gray-200'
              }`}
            >
        ...
        <table className={`min-w-full border rounded-lg ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
          ...
          <tbody>
            {orders.length === 0 ? (
              <tr>
                <td colSpan="5" className={`text-center py-8 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                  No orders found.
                </td>
              </tr>
            ) : orders.map((order, index) => (
